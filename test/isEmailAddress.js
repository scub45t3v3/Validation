(function() {
  ...
  describe('#isEmailAddress', function() {
    ...
    it('should return true for valid email address strings', function() {
      unit.bool(isEmailAddress('email@example.com')).isTrue().bool(isEmailAddress('firstname.lastname@example.com')).isTrue().bool(isEmailAddress('email@subdomain.example.com')).isTrue().bool(isEmailAddress('firstname+lastname@example.com')).isTrue().bool(isEmailAddress('disposable.style.email.with+symbol@example.com')).isTrue().bool(isEmailAddress('other.email-with-hyphen@example.com')).isTrue().bool(isEmailAddress('fully-qualified-domain@example.com')).isTrue().bool(isEmailAddress('user.name+tag+sorting@example.com')).isTrue().bool(isEmailAddress('example-indeed@strange-example.com')).isTrue().bool(isEmailAddress('x@example.com')).isTrue().bool(isEmailAddress('1234567890@example.com')).isTrue().bool(isEmailAddress('email@example-one.com')).isTrue().bool(isEmailAddress('_______@example.com')).isTrue().bool(isEmailAddress('email@example.name')).isTrue().bool(isEmailAddress('email@example.museum')).isTrue().bool(isEmailAddress('email@example.co.jp')).isTrue().bool(isEmailAddress('#!$%&\'*+-/=?^_`{}|~@example.org')).isTrue();
      return null;
    });
    it('should return false for invalid strings', function() {
      unit.bool(isEmailAddress('Abc.example.com')).isFalse().bool(isEmailAddress('A@b@c@example.com')).isFalse().bool(isEmailAddress('a"b(c)d,e:f;g<h>i[j\k]l@example.com')).isFalse().bool(isEmailAddress('just"not"right@example.com')).isFalse().bool(isEmailAddress('this is"not\allowed@example.com')).isFalse().bool(isEmailAddress('this\ still\"not\\allowed@example.com')).isFalse().bool(isEmailAddress('john..doe@example.com')).isFalse().bool(isEmailAddress('john.doe@example..com')).isFalse().bool(isEmailAddress(' email@example.com')).isFalse().bool(isEmailAddress('email@example.com ')).isFalse().bool(isEmailAddress('" "@example.org')).isFalse().bool(isEmailAddress('1234567890123456789012345678901234567890123456789012345678901234+x@example.com')).isFalse().bool(isEmailAddress('email@[123.123.123.123]')).isFalse().bool(isEmailAddress('user@[2001:DB8::1]')).isFalse().bool(isEmailAddress('"very.(),:;<>[]\".VERY.\"very@\\ \"very\".unusual"@strange.example.com')).isFalse().bool(isEmailAddress('user@localserver')).isFalse(); // technically valid but discouraged // technically valid but discouraged // technically valid but discouraged // technically valid but discouraged
      return null;
    });
